...
// runAgentCreate writes a markdown agent definition into .opencode/agent/.
...
func runAgentCreate(a *clix.Args) error {
	...
	if name == "" {
		return fmt.Errorf("agent create: an agent name is required (opencode agent create <name>)")
	}
