...
func runCommand() *clix.Command {
	flags := []clix.Flag{
		...
		{Name: "attach", Kind: clix.KindString, Describe: "attach to a running opencode server (e.g., http://localhost:4096)"},
...
func runRunCommand(a *clix.Args) error {
	...
	if a.String("attach") != "" {
		return notImplemented("opencode run --attach")
	}
	if a.String("command") != "" {
		return notImplemented("opencode run --command")
	}
...
// "opencode" invocation, serving the already-booted stack and attached to
...
func runInteractiveViaTUI(ctx context.Context, stack *stack, sessionID string, a *clix.Args) error {
	...
	go func() {
		srv := &server.Server{Session: stack.Service, Bus: stack.Bus, Permissions: stack.Permissions, Models: stack.Models, Agents: stack.Agents, Config: stack.Config, MCP: stack.MCP, Jobs: stack.Jobs, Questions: stack.Questions, Skills: stack.Skills, LSP: stack.LSP}
		server.ServeOn(listener, srv.Mux())
	}()
	opts := tui.RunOptions{DefaultModel: stack.ProviderID + "/" + stack.ModelID, SessionID: sessionID}
	if err := tui.Run(ctx, client.New("http://"+listener.Addr().String()), "opencode-dark", opts); err != nil {
