...
func runMCPList(a *clix.Args) error {
	...
	if len(servers) == 0 {
		...
		fmt.Println("Add servers with: opencode mcp add")
		return nil
	}
...
func runMCPAuth(a *clix.Args) error {
	...
	if name == "" {
		names := oauthServers(servers)
		if len(names) == 0 {
			return &usageError{msg: "No OAuth-capable MCP servers configured. Remote servers support OAuth by default; add one with: opencode mcp add"}
		}
...
func runMCPAdd(a *clix.Args) error {
	...
	if name == "" {
		...
		return &usageError{msg: "usage: opencode mcp add <name> --url <url> | -- <command...> (interactive prompts are not yet implemented in the Go port)"}
	}
...
// addMCPToGlobalConfig writes the server into the global opencode.json,
...
func addMCPToGlobalConfig(name string, server mcp.ServerConfig) (string, error) {
	...
	candidates := []string{filepath.Join(dir, "opencode.json"), filepath.Join(dir, "opencode.jsonc")}
	path := candidates[0]
	raw := "{}"
