...
func networkFlags() []clix.Flag {
	return []clix.Flag{
		...
		{Name: "mdns-domain", Kind: clix.KindString, Default: "opencode.local", Describe: "custom domain name for mDNS service (default: opencode.local)"},
...
func authHeaderFlags() []clix.Flag {
	return []clix.Flag{
		{Name: "password", Aliases: []string{"p"}, Kind: clix.KindString, Describe: "basic auth password (defaults to OPENCODE_SERVER_PASSWORD)"},
		{Name: "username", Aliases: []string{"u"}, Kind: clix.KindString, Describe: "basic auth username (defaults to OPENCODE_SERVER_USERNAME or 'opencode')"},
	}
}
