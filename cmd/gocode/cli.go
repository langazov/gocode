...
func runMain(argv []string) int {
	...
	if printLogs {
		os.Setenv("GOCODE_PRINT_LOGS", "1")
	}
	if logLevel != "" {
		os.Setenv("GOCODE_LOG_LEVEL", logLevel)
	}
	if pure {
		os.Setenv("GOCODE_PURE", "1")
	}
	os.Setenv("AGENT", "1")
	os.Setenv("GOCODE", "1")
...
func newRootCommand() *clix.Command {
	root := &clix.Command{
		Name:        "gocode",
		Describe:    "start gocode tui",
		Positionals: []clix.Positional{{Name: "project", Describe: "path to start gocode in"}},
		Flags:       rootTuiFlags(),
		Run:         runRootTui,
		Sub: []*clix.Command{
			acpCommand(),
...
func helpCommand() *clix.Command {
	return &clix.Command{
		...
		Run: func(a *clix.Args) error {
			root := newRootCommand()
			clix.PrintHelp(os.Stdout, root, []string{"gocode"})
			return nil
		},
	}
}
