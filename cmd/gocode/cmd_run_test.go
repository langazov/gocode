...
// TestRunCommandNonInteractive is an end-to-end smoke test for "gocode run
...
func TestRunCommandNonInteractive(t *testing.T) {
	...
	t.Setenv("GOCODE_AUTH_CONTENT", `{"faketest":{"type":"api","key":"k"}}`)
...
// thinking in the go port": "gocode run --variant high" against an
...
func TestRunCommandVariantEnablesThinking(t *testing.T) {
	...
	t.Setenv("GOCODE_DISABLE_MODELS_FETCH", "true")
	t.Setenv("GOCODE_AUTH_CONTENT", `{"faketest":{"type":"api","key":"k"}}`)
	t.Setenv("GOCODE_CONFIG_CONTENT", `{"model":"faketest/thinking-model"}`)
