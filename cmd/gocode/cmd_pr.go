...
// the gh CLI to fetch and check out a PR branch, then re-execs gocode.
func prCommand() *clix.Command {
	return &clix.Command{
		Name:        "pr",
		Describe:    "fetch and checkout a GitHub PR branch, then run gocode",
...
func runPR(a *clix.Args) error {
	...
	if len(output) > 0 {
		...
		if json.Unmarshal(output, &info) == nil {
			if m := sessionLinkRe.FindStringSubmatch(info.Body); m != nil {
				fmt.Printf("Found gocode session: %s\n", m[0])
				fmt.Println("Importing session...")
				imp := exec.Command("gocode", "import", m[0])
	...
	run := exec.Command("gocode", args...)
	run.Stdin, run.Stdout, run.Stderr = os.Stdin, os.Stdout, os.Stderr
	if err := run.Run(); err != nil {
		if exitErr, ok := err.(*exec.ExitError); ok {
			return fmt.Errorf("gocode exited with code %d", exitErr.ExitCode())
		}
		return err
	}
	return nil
